...
- **Standalone backtester** (`backtesting/`) — simulates the prediction module against historical minute-level data while markets are closed; starting cash and ticker universe are configurable via `.env`
  ...
  - **Live HTML dashboard** — `backtest.py` serves `backtest_ui.html` at `http://localhost:8787/` while running; vanilla-JS page polls `/api/state` every 2 seconds and renders a **simulated NYC clock** with a market-open/closed badge (based on NYSE hours 09:30–16:00 ET, Mon–Fri, excluding holidays), headline cards (NAV, cash, total P&L, positions, executed/skipped counts, cycles), a **portfolio value line chart** (pure canvas, no libraries — green when at net gain, red when at net loss, with a dashed breakeven line at initial cash and a shaded fill between the line and the baseline), a holdings table (ticker, shares, avg buy, last price, market value, unrealised P&L), and a reverse-chronological trade history with NYC-converted timestamps — accessible from any browser with no build step
